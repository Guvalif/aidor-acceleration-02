...
基礎課題を、`if` 命令を使用せずに実装してみましょう。

**A.**

いくつか方法はありますが、今回は `文字列` と `真偽値` の掛け算を利用した例を紹介します。
...
- `(digitalRead(MOTION_PIN) == 1) * 'Yes'` は、人感センサが反応したとき `Yes`
- `(digitalRead(MOTION_PIN) == 0) * 'No'` は、人感センサが反応していないとき `No`

となります。

さらに、`文字列 + 文字列` ⇒ `文字列` であることを利用すると、以下のような記述が実装例の1つとなります：
...
# =============================================================================
while True:
    print( (digitalRead(MOTION_PIN) == 1) * 'Yes' + (digitalRead(MOTION_PIN) == 0) * 'No' )
...
`関数` という仕組みを使って、ボリュームの値をPWMの値に変換する処理を、独自の命令として定義してみましょう。
